...
def detect_comma_or_dot_comma(file_path):
    csv_file_str = open(file_path, 'r', encoding='utf-8').read()
...
def clean_person(df,row,person_columns_list):
    rut = only_numerics(df[person_columns_list[3]][row])
    valid = len(rut) > 5
    if valid:
        ...
        phone = only_numerics(df[person_columns_list[5]][row])
...
def create_answers(df,question_df,person_columns_list,survey_obj):
    for row in df.index[2:]:
        #Create Person
        person_atr_list, valid = clean_person(df,row,person_columns_list)
        if valid:
            try:
        ...
        for column in question_df.index[person_columns_list[-1]+1:]:
            ...
            if question_df.type[column] == 'Dropdown questions':
                ...
                answer_obj, created= Answer.objects.get_or_create(survey_question_answer=survey_question_answer_obj,
...
def create_questions(question_df, survey_obj):
...
def create_models_from_csv(df,survey_obj):
    ...
    for column in df.columns:
        ...
        if (not pd.isna(first_cell)) and pd.isna(second_cell):
            ...
            first_row_is_question = False
        create_questions(questions_df,survey_obj)
    ...
    def post(self, *args, **kwargs):
        ...
        if survey_form.is_valid():
            ...
            if formset.is_valid():
                ...
                survey_df = pd.read_csv(csv_file_path, delimiter=detect_comma_or_dot_comma(csv_file_path), encoding='utf8',
                                        header=None)

                question_df, person_column_list = create_models_from_csv(survey_df, survey_obj)
                create_answers(survey_df,question_df, person_column_list, survey_obj)
